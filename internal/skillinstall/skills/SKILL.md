...
If both exist, the repository is already (partially or fully) migrated. **Do not run `docs-ssot migrate` on generated output files.** Instead, skip to Step 5 and run `docs-ssot validate` to assess the current state.
...
pre-push:
  commands:
    docs-check:
      glob: "{template/**/*.md,docsgen.yaml}"
      run: |
        docs-ssot build
        OUTPUTS=$(yq -r '.targets[].output' docsgen.yaml | tr '\n' ' ')
        if ! git diff --quiet $OUTPUTS template/INDEX.md; then
